      ...
      Object.entries(activities).forEach(([name, details]) => {
        ...
        const participantsMarkup = details.participants.length
          ? `<div class="participants"><strong>Participants:</strong><ul>${details.participants
              .map((participant) => `<li><span class="participant-name">${participant}</span><button class="delete-participant" data-activity="${name}" data-email="${participant}" aria-label="Remove ${participant}">✕</button></li>`)
              .join("")}</ul></div>`
          : `<p class="participants-empty"><strong>Participants:</strong> <em>No signups yet</em></p>`;
